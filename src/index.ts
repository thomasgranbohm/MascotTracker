...
namespace Logger {
	...
	export const debug = (message?: any, ...optionalParams: any[]) =>
		...
		console.debug(
			green(new Date().toLocaleString("sv")),
			format(message, ...optionalParams)
		);
...
if (!API_KEY || !SPREADSHEET_ID || !WEBHOOK_LINK) {
	Logger.error(
		"Please fill in the every environment variable before running!"
	);
	process.exit(1);
}
...
const executeWebhook = async (
	...
	try {
		...
		if (err.isAxiosError && err.response) {
			Logger.error("Error when executing webhook: %o", err.response);
		} else {
			Logger.error("Got regular error: %o", err);
		}
	}
};
...
const updateMessage = async (key: string, value: OnlineType) => {
	...
	if (!messageId)
		throw new Error(red("Could not find messageId with " + key));
	...
	rows
		...
		.map(
			(row): Mascot => ({
				name: row["Mascot"],
				online:
					"Enter the server below:" in row &&
					!!row["Enter the server below:"] &&
					row["Enter the server below:"] !== "Offline"
						? row["Enter room override below:"].startsWith(
								"Last visited"
						  )
							? [row["Enter the server below:"]]
							: [
		...
		try {
			...
			if (err.isAxiosError && err.response) {
				if (err.response.status === 429) {
					...
					Logger.debug(
						"Temporary Google API Error %d",
						err.response.status
					);
				} else {
					Logger.error(JSON.stringify(err.response.data, null, 4));
				}
			} else {
