...
const generateEmbed = (mascot, online = false) => ({
	title: `${mascot.name} went ${online ? `online in ${mascot.server}!`: "offline."}`,
...
const executeWebhook = (data) => axios.post(webhook_link, data, {
	headers: {
...
const main = async () => {
	...
	const run = async () => {
		...
		if (prevOffline.length === 0 && prevOnline.length === 0) prevOffline = mascots;

		const online = mascots
			.filter((o) => o.server !== "" && o.server !== "Offline")
			.filter((o) => !prevOnline.find((mascot) => mascot.name === o.name)),
			offline = mascots
			.filter((o) => o.server === "Offline")
			.filter((o) => !prevOffline.find((mascot) => mascot.name === o.name));
		...
		let date = new Date();
		offline.map((mascot) => `${date.toISOString()} ${mascot.name} went offline.`).forEach(console.log)
		online.map((mascot) => `${date.toISOString()} ${mascot.name} went online in ${mascot.server}!`).forEach(console.log)
		...
		prevOnline = mascots.filter((o) => o.server !== "" && o.server !== "Offline");
